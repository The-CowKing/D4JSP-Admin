        ...
        <div>
          ...
          <div style={{ ...cardStyle, marginBottom: 16, border: '1px solid rgba(245,158,11,0.18)' }}>
            ...
            {[
              ...
              { id: '060', title: 'Escrow Ship Deadline + Cron Expiry (Phase 1)', desc: 'Foundation for the 6h ship deadline + symmetric buyer-confirm deadline + warning ladder + auto-cancel/auto-confirm cron. Adds escrow.{ship_deadline, warning_3h_sent_at, warning_1h_sent_at, extended_count, shipped_at, confirm_warning_3h_sent_at, confirm_warning_1h_sent_at, confirm_extended_count} columns + users.{consecutive_bad_given, trading_locked, sms_enabled placeholder, sms_phone}. Adds messages.{type, payload, action_state} for system_trade lifecycle messages. Inserts 8 system_config rows (ship_deadline_hours=6, penalty_strikes_to_lock=3, extension_fee_fg=25, extension_hours=6, extension_fee_destination=buyer, max_extensions=2, confirm_deadline_hours=6, buyer_confirm_timeout_action=auto_confirm). Creates _process_expired_escrows() SECURITY DEFINER fn covering BOTH ship-side timeout (refund buyer + penalty seller) and buyer-confirm timeout (auto_confirm releases FG to seller). Schedules pg_cron escrow_expiry_tick every 5 minutes. Idempotent. Application-side helpers (push send, email fanout, mark-shipped endpoint, extension fee endpoint) ship in subsequent code commits.' },
              { id: '061', title: 'BattleTag Trade Gate (#138)', desc: 'Adam: "that message for the trade should also include users in game contact method too". Inserts system_config.escrow.require_battletag_for_trading=true so create-thread + initiate-escrow gate on a linked Battle.net account before listing/buying. lib/tradeGuards.js (assertBattletagLinked + getBattletag + formatContactLines) reads this knob and is wired into both endpoints. system_trade message body now includes both parties\' BattleTags so they can coordinate in-game. Toggle to false in Trade Settings to disable the gate (testing / phased rollout).' },
            ].map(m => {
